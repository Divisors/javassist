...
public abstract class TypeData {
	...
	public static class TypeVar extends AbsTypeVar {
		...
		public int dfs(List<TypeData> preOrder, int index, ClassPool cp) throws NotFoundException {
			...
			if (visited == smallest) {
				ArrayList<TypeData> scc = new ArrayList<>(); // strongly
															// connected
															// component
				TypeVar cv;
				do {
		...
		private void fixTypes(List<TypeData> scc, ClassPool cp) throws NotFoundException {
			HashSet<String> lowersSet = new HashSet<>();
		...
		private String fixTypes2(List<TypeData> scc, HashSet<String> lowersSet, ClassPool cp)
			...
			if (lowersSet.size() == 0)
				...
				if (cc.getSuperclass() == null || isObjectArray(cc))
					cc = fixByUppers((List<TypeVar>)(List<?>)scc, cp, new HashSet<TypeVar>(), cc);
		...
		private CtClass fixByUppers(List<TypeVar> users, ClassPool cp, HashSet<TypeVar> visited, CtClass type)
				throws NotFoundException {
			if (users == null)
				return type;
