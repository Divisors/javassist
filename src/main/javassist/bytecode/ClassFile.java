...
public final class ClassFile {
    ...
    private void testExistingField(String name, String descriptor) throws DuplicateMemberException {
        for (FieldInfo finfo : fields)
        	if (finfo.getName().equals(name))
        		throw new DuplicateMemberException("duplicate field: " + name + " in " + getName());
    }
    ...
    private void testExistingMethod(MethodInfo newMinfo) throws DuplicateMemberException {
        ...
        ListIterator<MethodInfo> methodIterator = this.methods.listIterator();
        while (methodIterator.hasNext())
            if (isDuplicated(newMinfo, newMinfo.getName(), descriptor, methodIterator.next(), methodIterator))
                throw new DuplicateMemberException("duplicate method: " + newMinfo.getName() + " in " + this.getName());
    }

	private static boolean isDuplicated(MethodInfo newMethod, String newName, String newDesc, MethodInfo minfo,
